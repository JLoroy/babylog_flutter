...
| First recording | Record a synthetic baby-care event and send it. | Screenshot of recording/send flow. | TODO for the full in-app microphone flow. Direct redacted endpoint smoke with synthetic audio confirms the temporary reviewer BYOK key can transcribe audio and extract a `bottle` event through the same endpoint/model family; evidence: `docs/qa-evidence/2026-05-07-openai-byok-endpoint-smoke.json`. |
