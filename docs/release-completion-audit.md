...
| Flutter tests | `flutter test` passed locally with 11 tests and in latest remote CI run `25467216242`. | Passing locally and remotely | Expand coverage as release risk changes. |
| Android debug build | `flutter build apk --debug` passed in latest remote CI run `25467216242`; local release APK/AAB builds for `1.0.7+8` also passed. | Passing locally and remotely | Keep remote CI green before each rollout. |
