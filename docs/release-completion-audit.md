...
| Professional release gate | `docs/release-verification.md`, `docs/play-release-runbook.md`, `.github/workflows/flutter-ci.yml`, local tests, and green remote runs including `25424811883` for commit `298e983`, `25425216927` for commit `2bb528a`, and `25427451856` for commit `587f230`. | Passing for current main | Continue keeping remote CI green after each release slice. |
...
| Android debug build | `flutter build apk --debug` is recorded passing locally; remote CI run `25427451856` built the debug APK on `main`. | Passing locally and remotely | Release AAB still must be uploaded to Play Console. |
