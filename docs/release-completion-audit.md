...
| Professional release gate | `docs/release-verification.md`, `docs/play-release-runbook.md`, `.github/workflows/flutter-ci.yml`, local tests, and green remote runs including `25465179981` for commit `0c64741` and `25465432676` for current tip commit `7d208b0`. Latest CI `25465432676` passed Firebase rules in 40s and Analyze/test in 6m21s, including formatting, analyzer, Flutter tests, Play/docs validators, and Android debug APK build. | Passing for current main | Continue keeping remote CI green after each release slice. |
| Flutter formatting | `dart format --set-exit-if-changed lib test` passed locally and in latest remote CI run `25465432676`. | Passing | Keep enforced in CI. |
| Flutter analyzer | `flutter analyze --no-fatal-infos` passed locally with 25 info-level issues and in latest remote CI run `25465432676`. | Passing locally and remotely | Info-level lint debt remains. |
| Flutter tests | `flutter test` passed locally with 10 tests and in latest remote CI run `25465432676`. | Passing locally and remotely | Expand coverage as release risk changes. |
| Android debug build | `flutter build apk --debug` passed in latest remote CI run `25465432676`; local release APK/AAB builds for `1.0.7+8` also passed. | Passing locally and remotely | Keep remote CI green before each rollout. |
