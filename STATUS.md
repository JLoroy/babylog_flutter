...
- GitHub Actions run `25450549774` for commit `991c90b` passed both jobs after shortening the Play reviewer instructions: Analyze/test completed in 6m32s including the private-notes generator test and Android debug APK build, and Firebase rules completed in 25s.
- Justin reported that the Play Console upload now appears as version code 7 and that App access notes were added. Release notes were updated for version `1.0.6+7` to explicitly call out the shared-assistant join fix while retaining security, deletion, local OpenAI key, policy page, and signing notes.
